function handleSaveLayout() {
    var t = $(".demo").html();
    t != window.demoHtml && (function() {return;}, window.demoHtml = t)
}
...
function gridSystemGenerator() {
    $(".lyrow .preview input").bind("keyup", function() {
        var t = 0,
            ...
            n = $(this).val().split(" ", 12);
        $.each(n, function(n, s) {
            i || (parseInt(s) <= 0 && (i = !0), t += parseInt(s), e += '<div class="col-xs-' + s + ' column"></div>')
...
function configurationElm(t, e) {
    ...
    //============= YANNI SECTION ================================
    demo.on("input", ".configuration > .yanni-control-label", function(t) {
        t.preventDefault();
        updateLabelAndNameForFormElement($(this));
    });

    demo.on("input", ".configuration > .yanni-control-dropdown", function(t) {
        t.preventDefault();
        try {
            var config = JSON.parse($(this).val());
            var context = $(this).parent().next().next();
            $('select', context).select2(config);
            $('input[type=hidden]', context).val(JSON.stringify(config));
        } catch (e) {
            console.log(e);
        }
    });

    $("#yanni-container").on('input', '.yanni-form-action', function (t) {
        t.preventDefault();
        var action = 'action="' + $(this).val() + '"';
        formWrapperOpen = formWrapperOpen.replace(/(action=".*")/g, action);
    });
    //============= YANNI SECTION ================================
}

function setNameForInput(name, context) {
    name = name.substring(0, name.length - 1);

    name += randomString(10);
    $('.yanni-name', context).attr("name", name);
}

function updateLabelAndNameForFormElement(e) {
    var label = e.val();

    var context = e.parent().next().next();

    $('.yanni-label', context).text(label);

    var name = $('input', context).attr("name");

    if (name === INPUT_DEFAULT_NAME) {
        setNameForInput(name, context);
    }
}
...
function downloadLayoutSrc() {
    ...
    t.find(".preview, .configuration, .drag, .remove").remove(),
        //============= YANNI SECTION ================================
        t.find(".yanni-form-action").remove(),
        //============= YANNI SECTION ================================
    ...
    $("#download-layout .container").length > 0 && changeStructure("row-fluid", "row"),
        ...
        //============= YANNI SECTION ================================
        $('textarea', "#layoutitModal").text(formWrapperOpen + formatSrc + formWrapperClose);
        //============= YANNI SECTION ================================
}
if (! function(t, e) {
...
//============= YANNI SECTION ================================
const INPUT_DEFAULT_NAME = "input_0";
var formWrapperOpen = '<form class="form-horizontal" role="form" enctype="multipart/form-data" method="post" action="{{ACTION_NAME}}">';
var formWrapperClose = '<button type="submit" class="btn btn-default">Submit</button></form>';
//============= YANNI SECTION ================================